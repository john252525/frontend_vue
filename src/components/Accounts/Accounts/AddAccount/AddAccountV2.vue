<template>
  <LoadModal :changeStationLoading="changeStationLoading" :stationLoading="stationLoading" />
  <div v-if="!stationLoading.loading" class="modal-overlay">
    <div class="modal-container">
      <div class="modal-header">
        ...
        <button class="close-btn accounts-addAccounts-close-button" @click="openModal">&times;</button>
      </div>

      <div class="modal-content">
        <!-- Group Select -->
        <div v-if="showElement('group')" class="form-group">
          <label v-if="getLabel('group')" class="accounts-addAccounts-group-label">{{ getLabel('group') }}</label>
          <div class="custom-select" ref="groupSelect">
            <div class="selected-option accounts-addAccounts-group-select" @click="toggleDropdown('group')">
            ...
            <teleport to="body" v-if="dropdownOpen.group">
              <div class="dropdown-options-global accounts-addAccounts-group-dropdown" :style="getDropdownStyle('group')">
                <div
        ...
        <div v-if="showElement('messenger')" class="form-group">
          <label v-if="getLabel('messenger')" class="accounts-addAccounts-messenger-label">{{ getLabel('messenger') }}</label>
          <div class="custom-select" ref="messengerSelect">
            <div class="selected-option accounts-addAccounts-messenger-select" @click="toggleDropdown('messenger')">
              <span>{{ getSelectedText('messenger') || "Выберите мессенджер" }}</span>
              <svg
                class="dropdown-icon"
            ...
            <teleport to="body" v-if="dropdownOpen.messenger">
              <div class="dropdown-options-global accounts-addAccounts-messenger-dropdown" :style="getDropdownStyle('messenger')">
                <div
        ...
        <div v-if="showElement('type')" class="form-group">
          <label v-if="getLabel('type')" class="accounts-addAccounts-crmType-label">{{ getLabel('type') }}</label>
          <div class="custom-select" ref="typeSelect">
            <div class="selected-option accounts-addAccounts-crmType-select" @click="toggleDropdown('type')">
            ...
            <teleport to="body" v-if="dropdownOpen.type">
              <div class="dropdown-options-global accounts-addAccounts-crmType-dropdown" :style="getDropdownStyle('type')">
                <div
        ...
        <div v-if="formValues.group === 'crm' && formValues.type" class="form-field">
          <label class="accounts-addAccounts-domain-label">Адрес аккаунта</label>
          <input
        ...
        <!-- Info messages -->
        <div 
          v-if="formValues.group === 'messenger' && formValues.messenger === 'whatsapp'" 
          class="info-message accounts-addAccounts-whatsapp-info"
        >
          <p>Привязать номер своего Whatsapp можно будет после создания аккаунта</p>
        </div>
        
        <div 
          v-if="formValues.group === 'messenger' && formValues.messenger === 'telegram'" 
          class="info-message accounts-addAccounts-telegram-info"
        >
          <p>Привязать свой аккаунт Telegram можно будет после создания аккаунта</p>
        </div>
      </div>

      <div class="modal-footer">
        <button class="cancel-btn accounts-addAccounts-cancel-button" @click="openModal">Отмена</button>
        <button class="submit-btn accounts-addAccounts-submit-button" @click="submitForm" :disabled="!isFormValid">
          Сохранить
        </button>
      </div>
    </div>
...
<script setup>
  ...
  const getSelectElement = (name) => {
    return formElements.value.find(el => el.element === 'select' && el.name === name);
  };
  
  const getLabel = (forAttr) => {
    const label = formElements.value.find(el => 
      el.element === 'label' && el.for === forAttr
    );
  ...
  const getSelectedText = (selectName) => {
    ...
    const option = formElements.value.find(el => 
  ...
  const getDropdownStyle = (name) => {
    const selectRef = name === 'group' ? groupSelect : 
                     name === 'messenger' ? messengerSelect : typeSelect;
  ...
  const handleClickOutside = (event) => {
    ...
    const isClickInsideDropdown = event.target.closest('.custom-select') || 
